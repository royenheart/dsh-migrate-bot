...
import { mergeReportKey } from '../../src/commands/idempotency.ts'
...
test('a merge this channel already reported is not reported to it again', async () => {
  const dir = pluginRepo()
  try {
    const { fetchImpl, calls } = githubStub({ merged: true })
    // The record the automatic path writes, as the state branch holds it.
    const seen = {
      tag: 'dsh-v0.1.5-rc.1',
      version: '0.1.5-rc.1',
      recordedAt: '2026-09-11T00:00:00Z',
      pending: { tag: 'dsh-v0.1.5-rc.1', version: '0.1.5-rc.1', pr: 7 },
      commands: [{
        key: mergeReportKey('acme/dsh-plugin-demo', 7),
        verb: 'feedback (merge)',
        at: '2026-09-12T03:00:00.000Z',
        channels: ['upgrade-skill'],
      }],
    }
    const result = await runFeedback({
      workdir: dir,
    ...
    const outcome = result.outcomes.find(entry => entry.channel === 'upgrade-skill')
    assert.equal(outcome?.status, 'skipped')
    assert.match(outcome?.status === 'skipped' ? outcome.reason : '', /already reported to it at 2026-09-12T03:00:00\.000Z/)
    assert.match(outcome?.status === 'skipped' ? outcome.reason : '', /--resend/)
    // Nothing was opened in anybody's repository, and no session was spent.
    assert.equal(calls.filter(call => call.method === 'POST').length, 0)

    // `--force` is how a maintainer means it, and then the channel delivers.
    const forced = await runFeedback({
      workdir: dir,
      config: mergedConfig(),
      env: { GITHUB_TOKEN: 'ghs_read', UPGRADE_TOKEN: 'ghp_write' },
      log: () => {},
    ...
    assert.equal(forced.outcomes.find(entry => entry.channel === 'upgrade-skill')?.status, 'delivered')
  } finally {
    rmSync(dir, { recursive: true, force: true })
  }
})

test('a merge reported to one channel still reaches the channel that was skipped', async () => {
  const dir = pluginRepo()
  try {
    const { fetchImpl, calls } = githubStub({ merged: true })
    const result = await runFeedback({
      ...
      config: parseConfig({
        feedback: {
          channels: {
            'upgrade-skill': { enabled: true, tokenEnv: 'UPGRADE_TOKEN' },
            'migrate-bot': { enabled: true, tokenEnv: 'BOT_TOKEN' },
          },
        },
      }),
      env: { GITHUB_TOKEN: 'ghs_read', UPGRADE_TOKEN: 'ghp_write', BOT_TOKEN: 'ghp_write' },
      log: () => {},
      seen: {
        tag: 'dsh-v0.1.5-rc.1',
        version: '0.1.5-rc.1',
    ...
    const byChannel = new Map(result.outcomes.map(outcome => [outcome.channel, outcome]))
    assert.equal(byChannel.get('upgrade-skill')?.status, 'skipped')
    assert.equal(byChannel.get('migrate-bot')?.status, 'delivered')
    // The one channel that had never received this merge is the one that got it.
    const opened = calls.filter(call => call.method === 'POST' && call.url.endsWith('/issues'))
    assert.equal(opened.length, 1)
    assert.match(opened[0]?.url ?? '', /royenheart\/dsh-migrate-bot/)
  } finally {
    rmSync(dir, { recursive: true, force: true })
  }
})

test('a reason from a classifier cannot add a line to a log or a reply', async () => {
  const dir = pluginRepo()
  try {
    const { fetchImpl } = githubStub({ merged: true })
    const lines: string[] = []
    // A held draft's reason is the model's own text, and it becomes a log line
    // (stdout, where the runner parses workflow commands) and a line of the reply.
    const held = '```json\n{"decisions":[{"slug":"web-server-rename","post":false,"reason":"already requested\\n::add-mask::not-a-secret\\n- @everyone"}]}\n```\n'
    const result = await runFeedback({
      workdir: dir,
      config: parseConfig({
        feedback: { channels: { 'harness-discussion': { enabled: true, tokenEnv: 'HARNESS_TOKEN' } } },
    ...
    assert.equal(result.outcomes.some(outcome => outcome.status === 'held'), true)
    for (const line of lines) {
      assert.equal(line.split('\n').length, 1, `a log line carried a newline: ${JSON.stringify(line)}`)
    }
    assert.equal(lines.some(line => line.includes('::add-mask::')), true)

    // The reply renders the same reason, and it is a comment.
    ...
    assert.equal(outcome.reply.split('\n').filter(line => line.startsWith('::add-mask::')).length, 0)
  } finally {
    rmSync(dir, { recursive: true, force: true })
  }
})

test('a ledger record read from the state branch cannot forge a line of the log', async () => {
  const dir = pluginRepo()
  try {
    const { fetchImpl } = githubStub({ merged: true })
    const lines: string[] = []
    // The record is the same one `status` renders, and the log line is written to
    // stdout where the runner parses workflow commands.
    const result = await runFeedback({
      workdir: dir,
      config: mergedConfig(),
      env: { GITHUB_TOKEN: 'ghs_read', UPGRADE_TOKEN: 'ghp_upgrade', HARNESS_TOKEN: 'ghp_harness' },
      log: (message: string) => lines.push(message),
      seen: {
        tag: 'dsh-v0.1.5-rc.1',
        version: '0.1.5-rc.1',
    ...
    assert.equal(result.ran, true)
    const line = lines.find(message => message.includes('already reported'))
    assert.notEqual(line, undefined)
    assert.equal(line?.split('\n').length, 1)
    assert.deepEqual(lines.filter(message => message.split('\n').some(part => part.startsWith('::'))), [])
    assert.match(line ?? '', /already reported at 2026-01-01T00:00:00\.000Z ::add-mask::for to upgrade-skill ::error::forged-channel/)
  } finally {
    rmSync(dir, { recursive: true, force: true })
  }
})
